...
%                 obj.Bintempfile=[obj.inputfile(1:end-4),'_temp.inp'];
            obj.Bintempfile=['RESULTS/',inp,'_temp.inp'];
...
%         end
        ...
        function value = getComputedQualityTimeSeries(obj,varargin)
            ...
            if find(strcmpi(varargin,'qualitySensingNodes'))
                value.QualitySensingNodes=zeros(totalsteps, length(sensingnodes));
                value.SensingNodesIndices=sensingnodes;
            end
            if find(strcmpi(varargin,'demandSensingNodes'))
                value.DemandSensingNodes=zeros(totalsteps, length(sensingnodes));
                value.SensingNodesIndices=sensingnodes;
            end
            ...
            while (tleft>0)||(t<obj.getTimeSimulationDuration)
                ...
                if find(strcmpi(varargin,'demandSensingNodes'))
                    value.DemandSensingNodes(k,:)=obj.getNodeActualDemandSensingNodes(varargin{3});
                end
                tleft = obj.nextQualityAnalysisStep;
                k=k+1;
